    ...
    if folder_id and user:
        ...
        if folder and folder.data:
            ...
            if 'files' in folder.data:
                ...
                # Always merge folder knowledge into request files so folder-scoped
                # chats behave like knowledge was attached directly to the chat.
                # Keep the metadata copy as well so builtin tools can still inspect
                # folder knowledge in native function-calling mode.
                form_data['files'] = list(
                    {
                        json.dumps(file, sort_keys=True): file
                        for file in [*allowed_files, *form_data.get('files', [])]
                    }.values()
                )
