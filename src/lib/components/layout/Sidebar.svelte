...
{#if $showSidebar}
	...
	>
		<div
			class="glass-panel my-auto mx-3 my-3 md:my-4 flex flex-col justify-between h-[calc(100dvh-1.5rem)] md:h-[calc(100dvh-2rem)] max-h-[calc(100dvh-1.5rem)] md:max-h-[calc(100dvh-2rem)] w-[calc(var(--sidebar-width)-0.75rem)] overflow-x-hidden scrollbar-hidden z-50 rounded-[1.85rem] {$showSidebar
				? ''
				: 'invisible'}"
		>
			<div
				class="sidebar px-3 pt-3 pb-2 flex justify-between space-x-1 text-gray-600 dark:text-gray-400 sticky top-0 z-10"
			>
				<a
					class="notebook-icon-button flex items-center rounded-xl size-8.5 h-full justify-center no-drag-region"
					href="/"
					draggable="false"
				...
				>
					<button
						class="notebook-icon-button flex rounded-xl size-8.5 justify-center items-center {isWindows
							? 'cursor-pointer'
							: 'cursor-[w-resize]'}"
						on:click={() => {
							showSidebar.set(!$showSidebar);
				...
				<div
					class="{scrollTop > 0
						? 'visible'
						: 'invisible'} sidebar-bg-gradient-to-b bg-linear-to-b from-[rgba(255,250,244,0.96)] dark:from-[rgba(18,28,34,0.94)] to-transparent from-50% pointer-events-none absolute inset-0 -z-10"
				></div>
			</div>

			<div
				class="relative flex flex-col flex-1 overflow-y-auto scrollbar-hidden px-1.5 pb-3"
				on:scroll={(e) => {
			...
			>
				<div class="pb-1.5">
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						<a
							id="sidebar-new-chat-button"
							class="notebook-sidebar-button group grow flex items-center space-x-3 rounded-2xl px-2.5 py-2.5 outline-none"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
							...
							{#if meta && isMenuItemVisible(itemId)}
								...
								>
									<a
										...
										class="notebook-sidebar-button grow flex items-center space-x-3 rounded-2xl px-2.5 py-2.5"
										href={meta.href}
							...
							{#if $chats}
								{#each $chats as chat, idx (`chat-${chat?.id ?? idx}`)}
									{#if idx === 0 || (idx > 0 && chat.time_range !== $chats[idx - 1].time_range)}
										<div
											class="w-full pl-2.5 notebook-section-label {idx ===
											0
												? ''
												: 'pt-5'} pb-1.5"
										>
			...
			<div class="px-2.5 pt-2 pb-3 sticky bottom-0 z-10 sidebar">
				<div
					class="sidebar-bg-gradient-to-t bg-linear-to-t from-[rgba(255,250,244,0.98)] dark:from-[rgba(18,28,34,0.96)] to-transparent from-50% pointer-events-none absolute inset-0 -z-10"
				></div>
				<div class="flex flex-col font-primary">
					{#if $user !== undefined && $user !== null}
						...
						>
							<button
								type="button"
								class="notebook-sidebar-button flex items-center rounded-2xl py-2.5 px-2 w-full"
								aria-label={$i18n.t('User menu')}
							>
	...
	{#if !$mobile}
		<div
			class="relative flex items-center justify-center group border-l border-[rgba(204,190,171,0.66)] dark:border-[rgba(87,105,114,0.36)] hover:border-[rgba(176,158,136,0.86)] dark:hover:border-[rgba(135,156,167,0.46)] transition z-20"
			id="sidebar-resizer"
