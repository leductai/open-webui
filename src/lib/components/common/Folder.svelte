		...
		{#if collapsible}
			...
			>
				...
				<div
					...
					class="notebook-sidebar-button w-full group rounded-2xl relative flex items-center justify-between transition {buttonClassName}"
				>
					<button class="w-full py-2 pl-2.5 flex items-center gap-1.5 text-xs font-medium">
						{#if chevron}
							<div class=" p-[1px]">
								{#if open}
					...
					{#if onAdd}
						...
						>
							<Tooltip content={onAddLabel}>
								<button
									class="notebook-icon-button p-1 rounded-lg touch-auto"
									on:click={(e) => {}}
								>
