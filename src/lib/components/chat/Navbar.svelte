...
>
	<div class="flex items-center w-full px-1.5 pt-1.5">
		<div class="glass-panel flex max-w-full w-full mx-auto rounded-[1.65rem] px-2.5 md:px-3 py-2 bg-transparent">
				...
				{#if $mobile && !$showSidebar}
					...
					>
						<Tooltip content={$showSidebar ? $i18n.t('Close Sidebar') : $i18n.t('Open Sidebar')}>
							<button
								class="notebook-icon-button cursor-pointer flex rounded-xl"
								on:click={() => {
			...
			"
				>
					{#if showModelSelector}
						<div class="rounded-2xl bg-[rgba(255,248,238,0.72)] px-2 py-1 dark:bg-[rgba(24,36,42,0.78)]">
							<ModelSelector
								bind:selectedModels
				...
				<div class="self-start flex flex-none items-center text-gray-600 dark:text-gray-400">
					...
					{#if $user?.role === 'user' ? ($user?.permissions?.chat?.temporary ?? true) && !($user?.permissions?.chat?.temporary_enforced ?? false) : true}
						{#if !chat?.id}
							<Tooltip content={$i18n.t(`Temporary Chat`)}>
								<button
									class="notebook-icon-button flex cursor-pointer px-2 py-2 rounded-xl"
						...
						<Tooltip content={$i18n.t('New Chat')}>
							<button
								class=" flex {$showSidebar
									? 'md:hidden'
									: ''} notebook-icon-button cursor-pointer px-2 py-2 rounded-xl text-gray-600 dark:text-gray-400"
								on:click={() => {
									initNewChat();
								}}
								aria-label="New Chat"
					...
					{#if $user !== undefined && $user !== null}
						...
						>
							<button
								type="button"
								class="notebook-icon-button select-none flex rounded-xl p-1.5 w-full"
