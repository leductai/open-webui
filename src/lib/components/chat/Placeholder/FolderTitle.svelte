<script lang="ts">
	...
	const copyVoiceLinkHandler = async () => {
		const voiceUrl = `${window.location.origin}/voice?folder=${encodeURIComponent(folder.id)}`;
		await navigator.clipboard.writeText(voiceUrl);
		toast.success($i18n.t('Voice link copied'));
	};
