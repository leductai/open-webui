<script lang="ts">
	...
	let chatTasks = [];
	let initializedEmptyChat = false;
	...
	onMount(() => {
		...
		const pageSubscribe = page.subscribe(async (p) => {
			if (!chatIdProp && !initializedEmptyChat) {
				await tick();
				initializedEmptyChat = true;
				initNewChat();
			}
...
<div
	class="h-screen max-h-[100dvh] transition-width duration-200 ease-in-out {$showSidebar
		...
		: ' '} w-full max-w-full flex flex-col overflow-hidden"
	id="chat-container"
>
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col relative z-10">
			...
			<PaneGroup direction="horizontal" class="w-full h-full md:gap-3">
				<Pane defaultSize={50} minSize={30} class="h-full flex relative max-w-full flex-col">
					...
					<div
						id="chat-pane"
						class="flex flex-col flex-auto z-10 w-full @container overflow-auto panel-surface grain-overlay"
					>
						{#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || createMessagesList(history, history.currentId).length > 0}
							...
							{:else}
								<div class="px-2 pb-2 md:px-3 {dragged ? 'z-0' : 'z-10'}">
									<MessageInput
