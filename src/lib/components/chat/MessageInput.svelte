				...
				<div class="">
					...
					>
						...
						{#if messageQueue.length > 0}
							<div
								class="mb-1 mx-2 py-1 px-1.5 rounded-[1.4rem] bg-[rgba(255,252,247,0.92)] dark:bg-[rgba(24,36,42,0.82)] border border-[rgba(204,190,171,0.78)] dark:border-[rgba(87,105,114,0.38)] overflow-x-hidden overflow-y-auto max-h-[25vh]"
							>
						...
						<div
							...
							class="flex-1 flex flex-col relative w-full shadow-lg rounded-3xl border {$temporaryChatEnabled
								? 'border-dashed border-[rgba(204,190,171,0.82)] dark:border-[rgba(87,105,114,0.38)]'
								: 'border-[rgba(204,190,171,0.82)] dark:border-[rgba(87,105,114,0.38)]'} transition px-1 bg-white/5 dark:bg-gray-500/5 backdrop-blur-sm dark:text-gray-100"
								...
								<div class="ml-1 self-end flex items-center flex-1 min-w-0">
									...
									>
										<button
											...
											class="notebook-icon-button rounded-full size-8 flex justify-center items-center outline-hidden focus:outline-hidden shrink-0"
									...
									{#if showWebSearchButton || showImageGenerationButton || showCodeInterpreterButton || showToolsButton || showSkillsButton || (toggleFilters && toggleFilters.length > 0)}
										<div
											class="flex self-center w-[1px] h-4 mx-1 bg-[rgba(176,158,136,0.42)] dark:bg-[rgba(135,156,167,0.32)] shrink-0"
										/>
									{/if}
											...
											<div class="ml-1 flex gap-1.5 shrink-0">
												<Tooltip content={$i18n.t('Valves')} placement="top">
													<button
														...
														class="notebook-icon-button rounded-full size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
														on:click={() => {
									...
									{#if isActive && prompt === '' && files.length === 0}
										<div class=" flex items-center">
											<Tooltip content={$i18n.t('Stop')}>
												<button
													class="notebook-subtle-button transition rounded-full p-1.5"
													on:click={() => {
														stopResponse();
													}}
												>
										...
										{#if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														class="notebook-primary-button transition rounded-full p-1.5 self-center"
														type="button"
														on:click={async () => {
