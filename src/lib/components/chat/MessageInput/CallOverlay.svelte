<script lang="ts">
	...
	let rmsLevel = 0;
	$: isListening = !loading && !muted && !assistantSpeaking;
	$: listeningIntensity = Math.max(0.12, Math.min(1, rmsLevel * 18));
...
{#if $showCallOverlay}
	<div class="max-w-lg w-full h-full max-h-[100dvh] flex flex-col justify-between p-3 md:p-6">
		{#if camera}
			...
			>
				...
				{:else if loading || assistantSpeaking}
					...
					>
				{:else}
					<div class="voice-avatar-shell">
						{#if isListening}
							<div
								class="voice-listening-ring voice-listening-ring-outer"
								style={`opacity:${0.22 + listeningIntensity * 0.22}; transform: scale(${1.02 + listeningIntensity * 0.22});`}
							/>
							<div
								class="voice-listening-ring voice-listening-ring-inner"
								style={`opacity:${0.32 + listeningIntensity * 0.26}; transform: scale(${1.01 + listeningIntensity * 0.12});`}
							/>
						{/if}
						<div
							class=" {rmsLevel * 100 > 4
								...
								: rmsLevel * 100 > 2
									...
									: rmsLevel * 100 > 1
										? 'size-14'
										: 'size-12'} transition-all rounded-full bg-cover bg-center bg-no-repeat voice-avatar-core"
					...
					{:else}
						<div class="voice-avatar-shell">
							{#if isListening}
								<div
									class="voice-listening-ring voice-listening-ring-outer"
									style={`opacity:${0.18 + listeningIntensity * 0.22}; transform: scale(${1.02 + listeningIntensity * 0.18});`}
								/>
								<div
									class="voice-listening-ring voice-listening-ring-inner"
									style={`opacity:${0.26 + listeningIntensity * 0.24}; transform: scale(${1.01 + listeningIntensity * 0.1});`}
								/>
							{/if}
							<div
								class=" {rmsLevel * 100 > 4
									...
									: rmsLevel * 100 > 2
										...
										: rmsLevel * 100 > 1
											? 'size-44'
											: 'size-40'} transition-all rounded-full bg-cover bg-center bg-no-repeat voice-avatar-core"
								style={`background-image: url('${WEBUI_API_BASE_URL}/models/model/profile/image?id=${model?.id}&lang=${$i18n.language}&voice=true');`}
							/>
						</div>
					{/if}
		...
		<div class="flex flex-col items-center gap-4 pb-4 w-full">
			...
			>
				<div class="line-clamp-1 text-sm font-medium {isListening ? 'voice-listening-label' : ''}">
					{#if loading}
						{$i18n.t('Thinking...')}
					{:else if muted}
						{$i18n.t('Muted')}
...
<style>
	...
	.voice-avatar-core {
		...
		z-index: 2;
		box-shadow:
			0 18px 40px rgba(20, 39, 48, 0.14),
	...
	.voice-listening-ring {
		...
		background:
			radial-gradient(circle, rgba(82, 189, 216, 0.2) 0%, rgba(82, 189, 216, 0.12) 42%, transparent 70%);
		z-index: 1;
		animation: voice-listening-pulse 1.8s ease-in-out infinite;
		pointer-events: none;
	}

	.voice-listening-ring-inner {
		animation-duration: 1.25s;
	}

	.voice-listening-ring-outer {
		animation-duration: 2.1s;
	}

	.voice-listening-label {
		animation: voice-listening-label 1.35s ease-in-out infinite;
		color: rgb(30 64 175 / 0.92);
	}

	:global(.dark) .voice-listening-label {
	...
	@keyframes voice-listening-label {
		0%,
		100% {
			opacity: 0.72;
